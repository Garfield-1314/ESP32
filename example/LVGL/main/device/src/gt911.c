...
{
  if (rotation > ROTATION_NORMAL) return ESP_ERR_INVALID_ARG;
...
{
  if (index < 5) return dev->points[index];
  return (tp_point_t){0};
}
...
static tp_point_t read_point(gt911_dev_t *dev, uint8_t *data)
{
  ...
  switch (dev->rotation) {
    case ROTATION_NORMAL: x = dev->width - x; y = dev->height - y; break;
    case ROTATION_LEFT:   temp = x; x = dev->width - y; y = temp; break;
    case ROTATION_INVERTED: break;
    case ROTATION_RIGHT:  temp = x; x = y; y = dev->height - temp; break;
  }
...
static esp_err_t write_byte_data(gt911_dev_t *dev, uint16_t reg, uint8_t val)
{
  uint8_t write_buf[3] = {(uint8_t)(reg >> 8), (uint8_t)(reg & 0xFF), val};
