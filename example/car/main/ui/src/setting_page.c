...
static void setting_page_delete_cb(lv_event_t *e) { setting_page = NULL; }

static void logo_switch_cb(lv_event_t *e) {
  lv_obj_t *sw = lv_event_get_target(e);
  show_logo = lv_obj_has_state(sw, LV_STATE_CHECKED);
  model_nvs_save();
}
...
void create_setting_page(void) {
  ...
  // Logo显示开关
  lv_obj_t *logo_cont = lv_obj_create(setting_page);
  lv_obj_set_size(logo_cont, 280, 35);
  lv_obj_align(logo_cont, LV_ALIGN_TOP_MID, 0, 35);
  ...
  lv_label_set_text(logo_label, "Show Logo");
  lv_obj_set_style_text_color(logo_label, lv_color_white(), 0);
  lv_obj_align(logo_label, LV_ALIGN_LEFT_MID, 10, 0);

  lv_obj_t *logo_sw = lv_switch_create(logo_cont);
  lv_obj_align(logo_sw, LV_ALIGN_RIGHT_MID, -10, 0);
  if (show_logo) {
    lv_obj_add_state(logo_sw, LV_STATE_CHECKED);
  }
  lv_obj_add_event_cb(logo_sw, logo_switch_cb, LV_EVENT_VALUE_CHANGED, NULL);
  ...
  lv_obj_set_size(info_cont, 280, 200);
  lv_obj_align(info_cont, LV_ALIGN_TOP_MID, 0, 80);
