...
static void lvgl_touch_read_cb(lv_indev_drv_t *indev_drv,
  ...
  if (is_touched) {
    ...
    // 假设 raw_y 范围 0-240 -> 映射到 screen_x 0-320
    // 假设 raw_x 范围 0-320 -> 映射到 screen_y 0-240
    // 根据测试 raw_y 是反向的
    ...
    int16_t x_calc = 320 - (((240 - p.y) * 320) / 240);
    int16_t y_calc = 240 - ((p.x * 240) / 320);
...
void lvgl_event_task(void *arg) {
  ...
  /* Defer OTA check by ~2 seconds after LVGL starts so UI is fully ready */
  uint32_t startup_delay = 100;  /* 100 ticks × 20 ms = 2 s */
  ...
  for (;;) {
    ...
    /* Drain command messages from UART/tinyUSB — must be on LVGL thread */
    cmd_page_process_queue();

    /* Once, after startup delay: check for firmware and show dialog */
    if (!ota_checked) {
      if (startup_delay > 0) {
        startup_delay--;
      } else {
